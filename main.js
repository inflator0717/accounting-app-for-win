...
//获取用途数据
// ipcMain.on('get-usage', (event, accountType) => {
//   // 使用 __dirname 获取当前模块的目录名
//   const filePath = path.join(__dirname, 'data.xlsx');
//   console.log('File will be read from:', filePath);
...
//     const usages = [...new Set(records.map(record => record[3]))]; // 假设用途在第四列
...
//       return recordCategory === filter.category && recordDate >= startDate && recordDate <= endDate;
//     });
